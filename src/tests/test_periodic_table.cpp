...
TEST_CASE("periodic table", "[pte]")
{
  ...
  SECTION("select by atomic number")
  {
    ...
    test_select_value(db, "SELECT covalent_radius FROM periodic_table WHERE atomic_number = 9", 0.64);
  }
  ...
  SECTION("select by symbol")
  {
    ...
    test_select_value(db, "SELECT covalent_radius FROM periodic_table WHERE symbol = 'F'", 0.64);
  }
