...
static void mol_to_smarts(sqlite3_context* ctx, int /*argc*/, sqlite3_value** argv)
{
  sqlite3_value *arg = argv[0];
  ...
  if ( rc != SQLITE_OK ) {
    ...
    std::string smiles = RDKit::MolToSmarts(*mol);
    sqlite3_result_text(ctx, smiles.c_str(), -1, SQLITE_TRANSIENT);
  }
}

static void mol_from_smarts(sqlite3_context* ctx, int /*argc*/, sqlite3_value** argv)
{
  sqlite3_value *arg = argv[0];
  int value_type = sqlite3_value_type(arg);
  ...
  /* build the molecule blob repr from a text string */
  std::string smarts = (const char *)sqlite3_value_text(arg);
  std::unique_ptr<RDKit::ROMol> mol;

  try {
    mol.reset(RDKit::SmartsToMol(smarts));
  }
  catch (...) {
    chemicalite_log(
      SQLITE_ERROR,
...
int chemicalite_init_mol_formats(sqlite3 *db)
{
  ...
  if (rc == SQLITE_OK) rc = sqlite3_create_function(db, "mol_from_smarts", 1, SQLITE_UTF8 | SQLITE_DETERMINISTIC, 0, strict<mol_from_smarts>, 0, 0);
  ...
  if (rc == SQLITE_OK) rc = sqlite3_create_function(db, "mol_to_smarts", 1, SQLITE_UTF8 | SQLITE_DETERMINISTIC, 0, strict<mol_to_smarts>, 0, 0);
